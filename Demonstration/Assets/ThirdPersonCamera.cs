...
public class ThirdPersonCamera : MonoBehaviour
{
	...
	// Verify setup, initialise bookkeeping
	{
		...
		targetDistance = optimalDistance = (camera.transform.position - position()).magnitude;
	}
	
	Vector3 position(){
		
	return 	target.transform.position+new Vector3(0,1.5f,0);
	...
	// See if the camera touches the ground and adjust the camera distance if an object blocks the view
	{
		...
		Vector3 inverseLineOfSight = camera.transform.position - position();
		
		RaycastHit hit;
		if (Physics.SphereCast (position(), ViewRadius, inverseLineOfSight, out hit, optimalDistance, obstacleLayers))
		// Cast a sphere from the target towards the camera - using the view radius - checking against the obstacle layers
		{
			targetDistance = Mathf.Min ((hit.point - position()).magnitude, optimalDistance);
	...
	// Update camera position - specifics are delegated to camera mode functions
	{
		...
		else
		{
			...
			Vector3 movement = position() - lastStationaryPosition;
	...
	{
		Vector3 cameraForward = position() - camera.transform.position;
		...
		camera.transform.RotateAround (position(), Vector3.up, rotationAmount);
	}
	...
	// Control the camera via the mouse
	{
		...
		{
			camera.transform.RotateAround (position(), camera.transform.right, rotationAmount);
	...
	{
		Vector3 targetPosition = position() + (camera.transform.position - position()).normalized * targetDistance;
	...
	// Use gizmos to gain information about the state of your setup
	{
		...
		Gizmos.DrawLine (position(), position() + target.transform.forward);
